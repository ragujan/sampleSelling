...
if (isset($_POST["PG"])) {
    ...
    $predictnumofrows = DB::forsearch("SELECT * FROM subsampletype 
...
<div id="thesamplecontainer1" class="row thesamplecontainer1  ">
    <div class="col-12 appear">
        <div class="row">
            <?php
            $getMelodies = DB::forsearch("SELECT * FROM subsampletype 
            INNER JOIN samples ON samples.SubsampleID=subsampletype.subsampleID 
            WHERE subsampletype.sampleTypeID='1' LIMIT 8 OFFSET $pagenumber;");
            ...
            if ($getMelodiesrows >= 1) {
                for ($i = 0; $i < $getMelodiesrows; $i++) {
                    ...
                    $getImage = DB::forsearch("SELECT * FROM `sampleimages` 
                    WHERE `sampleID`='" . $melodyID . "' ;");

                    $getImagepath = $getImage->fetch_assoc();
                    $imagePath = $getImagepath["source_URL"];

                    $getaudio = DB::forsearch("SELECT * FROM `sampleaudio` 
                    WHERE `sampleID`='" . $melodyID . "' ;");
            ...
            <div class="col-6 offset-3">
                <div class="row">
                    <div class="col-2  text-center  d-grid ">
                        <button id="prev" class=" nextButton" onclick="nextfunction1(<?php echo $receivednumber - 1; ?>);">Prev</button>
                    </div>
                    <div class="col-8">
                        <?php
                    ...
                    <div class="col-2  text-center  d-grid">
                        <button id="next" class=" nextButton" onclick="nextfunction1(<?php echo $receivednumber + 1; ?>);"><?php echo  $receivednumber; ?></button>
                    </div>
                </div>
            </div>
        </div>
